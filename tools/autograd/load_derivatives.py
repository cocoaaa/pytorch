    ...
    def set_up_derivatives(defn_name, defn, declaration):
        ...
        def postprocess_fw_derivative(fw_derivatives, derivatives, args_with_derivatives):
            ...
            for fw_def in fw_derivatives:
                ...
                # Special check to avoid infinite recursion when extracting the primal of a dual
                # Tensor for forward AD. See "fw_primal(uint64_t level)" in Aten/templates/TensorBody.h
                # for more details
                if defn_name == "view" and "self" in required_inputs_primal:
                    raise RuntimeError("The forward grad formula for 'view' should not require the primal "
                                       "value of its input. This will lead to infinite recursion when "
                                       "extracting the primal of a dual Tensor.")
